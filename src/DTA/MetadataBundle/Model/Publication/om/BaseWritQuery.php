...
abstract class BaseWritQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'DTAMetadata', $modelName = 'DTA\\MetadataBundle\\Model\\Publication\\Writ', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \DTA\MetadataBundle\Model\Classification\CorpusQuery A secondary query class using the current class as primary query
     */
    public function useCorpusQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCorpus($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Corpus', '\DTA\MetadataBundle\Model\Classification\CorpusQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \DTA\MetadataBundle\Model\Workflow\SourceQuery A secondary query class using the current class as primary query
     */
    public function useSourceQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinSource($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Source', '\DTA\MetadataBundle\Model\Workflow\SourceQuery');
    }
