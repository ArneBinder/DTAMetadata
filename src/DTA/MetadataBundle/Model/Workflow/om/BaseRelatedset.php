...
abstract class BaseRelatedset extends BaseObject implements Persistent, \DTA\MetadataBundle\Model\table_row_view\TableRowViewInterface
{
    /**
     * Peer class name
     */
    const PEER = 'DTA\\MetadataBundle\\Model\\Workflow\\RelatedsetPeer';
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {
                $stmt = $con->query("SELECT nextval('relatedset_id_seq')");
                $row = $stmt->fetch(PDO::FETCH_NUM);
                $this->id = $row[0];
            } catch (Exception $e) {
