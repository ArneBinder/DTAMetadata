...
abstract class BaseWorkTag extends BaseObject implements Persistent, \DTA\MetadataBundle\Model\table_row_view\TableRowViewInterface
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if (null === $this->id) {
            try {
                $stmt = $con->query("SELECT nextval('work_tag_id_seq')");
                $row = $stmt->fetch(PDO::FETCH_NUM);
                $this->id = $row[0];
            } catch (Exception $e) {
    ...
    {
        if (isset($alreadyDumpedObjects['WorkTag'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['WorkTag'][$this->getPrimaryKey()] = true;
